...
use quote::quote;
use syn::{spanned::Spanned, Attribute, Error, Item, ItemEnum, Result};

use super::util::parse_bits_attr;
...
pub(crate) fn generate(item: Item) -> Result<TokenStream2> {
    ...
    let bits = bits_to_fit_enum(&e, bits_attr)?;
...
/// Calculate min number of bits to fit all enum variant discriminants, or use the value in #[bits = N] if specified.
fn bits_to_fit_enum(e: &ItemEnum, bits_attr: Option<(usize, &Attribute)>) -> Result<usize> {
    match bits_attr {
        Some((bits, attr)) => {
